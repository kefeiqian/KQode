...
- **Repo scope:** Optional. Accept catalog IDs or aliases from `references/repo-catalog.md`. Default to the first-scope repos.
...
When no question is supplied, investigate what happens after a user submits a prompt to each first-scope agent:
...
2. Fetch current upstream source and record requested URL, resolved URL, branch, commit SHA, fetch timestamp, and status.
3. Search and read source evidence using bounded budgets from `references/research-workflow.md`.
4. Cite every material observed-behavior claim with numbered references whose entries contain commit-pinned source links.
